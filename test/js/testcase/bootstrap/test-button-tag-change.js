...
(function () {
  ...
  describe('UICP01 Bootstrap形式のspan要素やdiv要素でのボタンが利用できること', function () {

    // 試験対象サンプルプログラム。
    var sampleFileName = PATH.BOOTSTRAP_BUTTON_TAG_CHANGE;
    var MSG_BOOTSTRAP_BUTTON_TAG_CHANGE = 'bootstrap_button_tag_change.';
    ...
    // ----------------------- テストケース -----------------------
    it('UICP0106 001 ボタンが4つ表示されていることを確認する', function (done) {
      this.timeout(0);
      ...
      m.executeSequentialWithDelay([
        ...
        function () {
          ...
          assert.isNotNull(div1, MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.isNotNull(span1, MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.isNotNull(div2, MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.isNotNull(span2, MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(div1Style, 'inline-block', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(span1Style, 'inline-block', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(div2Style, 'inline-block', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(span2Style, 'inline-block', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
        },
        function () {
          done();
        }
    ...
    // ----------------------- テストケース -----------------------
    it('UICP0106 007 divボタン2にEnterキーを押下してテキストボックスの値を確認する', function (done) {
      this.timeout(0);
      var div2 = testObj.doc.querySelector('#div2');
      ...
      m.executeSequentialWithDelay([
        ...
        function () {
          ...
          /**
           * 1)client-functionaltest/bootstrap/button-tag-change.htmlを開く。
           * 2)divボタン2を押下する。
           * 3)Enterキーを押下する
           */
        ...
        function () {
          ...
          assert.equal(keyupStatusValue, 'div2:keyup', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(keypressStatusValue, 'div2:keypress', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(keydownStatusValue, 'div2:keydown', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
        },
        function () {
          done();
        }
    ...
    // ----------------------- テストケース -----------------------
    it('UICP0106 008 spanボタン2にEnterキーを押下してテキストボックスの値を確認する', function (done) {
      this.timeout(0);
      var span2 = testObj.doc.querySelector('#span2');
      ...
      m.executeSequentialWithDelay([
        ...
        function () {
          ...
          assert.equal(keyupStatusValue, 'span2:keyup', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(keypressStatusValue, 'span2:keypress', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
          assert.equal(keydownStatusValue, 'span2:keydown', MSG_BOOTSTRAP_BUTTON_TAG_CHANGE);
        },
        function () {
          done();
        }
