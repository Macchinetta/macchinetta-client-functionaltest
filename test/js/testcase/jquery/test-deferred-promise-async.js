...
(function () {
  ...
  describe('ASNC01 jQuery形式で1秒毎にメッセージを出力できる', function () {

    // 試験対象サンプルプログラム。
    var sampleFileName = PATH.DEFERRED_PROMISE_ASYNC;
    var MSG_DEFERRED_PROMISE_ASYNC = 'jquery-deferred-promise-async.';
    ...
    // ----------------------- テストケース -----------------------
    it('ASNC0101 001 1秒毎にメッセージを出力できること', function (done) {
      this.timeout(0);
      ...
      m.executeSequentialWithSpecificDelay([
        ...
        },
        {
          fn: function () {
            ...
            assert.isNotNull(p1, MSG_DEFERRED_PROMISE_ASYNC);
            assert.isNull(p2, MSG_DEFERRED_PROMISE_ASYNC);
        ...
        },
        {
          fn: function () {
            ...
            var p3 = testObj.doc.querySelector('#deferred-area > p:nth-child(4)');
            assert.isNotNull(p1, MSG_DEFERRED_PROMISE_ASYNC);
            assert.isNotNull(p2, MSG_DEFERRED_PROMISE_ASYNC);
            assert.isNull(p3, MSG_DEFERRED_PROMISE_ASYNC);
            assert.equal(p2.textContent, '2', MSG_DEFERRED_PROMISE_ASYNC);
          },
          delay:1000
        },
        {
          fn: function () {
            ...
            assert.isNotNull(p1, MSG_DEFERRED_PROMISE_ASYNC);
            assert.isNotNull(p2, MSG_DEFERRED_PROMISE_ASYNC);
            assert.isNotNull(p3, MSG_DEFERRED_PROMISE_ASYNC);
            assert.equal(p3.textContent, '3', MSG_DEFERRED_PROMISE_ASYNC);
            done();
          },
          delay:1000
        }
